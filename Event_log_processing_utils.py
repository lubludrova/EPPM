...
def Extract_trace_and_temporal_features_label(tab):
    # Extract trace and compute the 4 timed features for each event
    lastcase = ''
    line = [] # to store all activities of each case
    firstLine = True
    ...
    lines_t3.append(times3)
    lines_t4.append(times4)
    return lines, lines_t, lines_t2, lines_t3, lines_t4, labels


def Extract_prefix_label(lines, lines_t, lines_t2, lines_t3, lines_t4, labels):
    step = 1
    sentences = []
    next_ope = []
    sentences_t = []
    ...
    end_ope_t = []
    y = []
    for line, line_t, line_t2, line_t3, line_t4, label in zip(lines, lines_t, lines_t2, lines_t3, lines_t4, labels):
        for i in range(2, len(line), step): # Keep only prefix with length min = 2
            sentences.append(line[0: i])
            sentences_t.append(line_t[0:i])
            sentences_t2.append(line_t2[0:i])
            ...
            #end_ope_t.append(line_t2[-1] - line_t2[i-1])
            y.append(label)
    return [sentences, sentences_t, sentences_t2, sentences_t3, sentences_t4], y #[next_ope, next_ope_t, end_ope_t]
